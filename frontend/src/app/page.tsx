      ...
      <section className="py-24 px-6 md:px-16 border-t border-[#2A2A2A]">
        <div className="max-w-[960px] mx-auto">
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] p-8 mb-8">
            ...
            <button
              onClick={() => {
                const bookmarkletCode = `javascript:(function(){if(!window.location.hostname.includes('suno')){alert('⚠️ Please run this bookmarklet while on suno.com');return;}let title='';let prompt='';let lyrics='';let artist='';let model='Suno';const titleEls=document.querySelectorAll('h1, [class*="title"], [class*="Title"]');for(const el of titleEls){const text=el.textContent.trim();if(text&&text.length<100&&!text.includes('Suno')&&!text.includes('Create')){title=text;break;}}const textareas=document.querySelectorAll('textarea');for(const ta of textareas){const text=ta.value||ta.textContent||'';const placeholder=ta.placeholder||'';if(placeholder.toLowerCase().includes('descri')||placeholder.toLowerCase().includes('prompt')){if(text&&!lyrics){prompt=text;}}else if(text.length>50){lyrics=text;}}if(!prompt&&lyrics){const lines=lyrics.split('\\n');if(lines.length>0){prompt='Song with lyrics: '+lines[0].substring(0,100)+'...';}}const buttons=document.querySelectorAll('button, a');for(const btn of buttons){const text=btn.textContent?.trim()||'';if(text.includes('@')&&text.length<30){artist=text.replace('@','');break;}}const versionMatch=document.body.textContent.match(/v[0-9.]+/i);if(versionMatch){model='Suno '+versionMatch[0];}else{model='Suno v4';}if(!title){title='Untitled Track';}if(!prompt){prompt='Generated with Suno AI';}const baseUrl=window.location.hostname.includes('localhost')?'http://localhost:3000/new':'https://inf8.vercel.app/new';const params=new URLSearchParams({title:title,aiPrompt:prompt,methodology:'AI-generated track with Suno. Review and describe your actual process.',model:model,artist:artist||'Your Name',aiModels:model,daws:'N/A',plugins:'N/A',hardware:'N/A',aiComp:'100',aiArr:'100',aiProd:'100',aiMix:'100',aiMaster:'100'});const url=\`\${baseUrl}?\${params.toString()}\`;window.open(url,'_blank');alert('✅ Opening ∞8 ARCH declaration form with Suno data!');})();`;
